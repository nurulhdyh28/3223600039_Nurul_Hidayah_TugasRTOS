...
void setup() {
  ...
  xTaskCreatePinnedToCore(taskEncoder,     "Encoder",  2048, NULL, 1, NULL, 1); // Prio 3 
  xTaskCreatePinnedToCore(taskServo,       "Servo",    2048, NULL, 1, NULL, 0); // Prio 2
  xTaskCreatePinnedToCore(taskStepper,     "Stepper",  4096, NULL, 1, NULL, 1); // Prio 3 
}
...
void taskOLED(void *pvParameters) {
  ...
  for (;;) {
    display.clearDisplay();
    display.setCursor(0, 0);
    display.println("Nurul Hidayah (039)");
    display.println(" ");
...
// Task 8: Memainkan Nada di Buzzer 
void taskBuzzer(void *pvParameters) {
  (void) pvParameters;
  for(;;) {
